...
export default function RecrutamentoPage() {
      ...
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-gray-500 mt-1">{pos.department?.name || 'Sem departamento'} &middot; {pos.location || 'Remoto'} &middot; {pos.type}</p>
            {pos.salaryMin && <p className="text-sm text-gray-500 mt-1">Faixa salarial: {currency(pos.salaryMin)} - {pos.salaryMax ? currency(pos.salaryMax) : '...'}</p>}
          </div>
          <div className="flex gap-2">
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[pos.status] || 'bg-gray-100'}`}>{statusLabels[pos.status] || pos.status}</span>
            {pos.status === 'OPEN' && (
              <button onClick={() => updatePositionStatus(pos.id, 'CLOSED')} className="text-xs px-3 py-1 bg-gray-200 rounded hover:bg-gray-300">Fechar Vaga</button>
            )}
            {pos.status === 'CLOSED' && (
        ...
        {pos.description && <p className="text-gray-700 text-sm whitespace-pre-wrap">{pos.description}</p>}
            ...
            <div key={app.id} className="bg-white border rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{app.candidate.email} {app.candidate.phone && `· ${app.candidate.phone}`}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${appStatusColors[app.status] || 'bg-gray-100'}`}>{appStatusLabels[app.status] || app.status}</span>
                  <select
                    value={app.status}
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      {showForm && tab === 'vagas' && (
        <form onSubmit={createPosition} className="bg-white border rounded-lg p-6 space-y-4">
      ...
      {showForm && tab === 'candidatos' && (
        <form onSubmit={createCandidate} className="bg-white border rounded-lg p-6 space-y-4">
            ...
            <div key={pos.id} onClick={() => loadPositionDetail(pos.id)} className="bg-white border rounded-lg p-4 cursor-pointer hover:border-green-300 transition-colors">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{pos.department?.name || 'Sem departamento'} &middot; {pos.location || 'Remoto'} &middot; {pos.type}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-sm text-gray-500">{pos._count.applications} candidatura(s)</span>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[pos.status] || 'bg-gray-100'}`}>{statusLabels[pos.status] || pos.status}</span>
                </div>
              </div>
            </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {candidates.map(c => (
                <tr key={c.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-500">{c._count?.applications || 0}</td>
                  <td className="px-4 py-3 text-sm text-gray-500">{new Date(c.createdAt).toLocaleDateString('pt-BR')}</td>
                </tr>
              ))}
            </tbody>
          </table>
