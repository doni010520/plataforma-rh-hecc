  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-green-950/50 backdrop-blur-lg rounded-lg border border-green-800/30 p-6">
        ...
        <div className="mt-4"><div className="flex justify-between text-sm text-gray-400 mb-1"><span>Progresso: {completedTasks}/{totalTasks} tarefas</span><span>{progress}%</span></div><div className="w-full bg-green-800/40 rounded-full h-3"><div className="bg-green-700 h-3 rounded-full transition-all" style={{ width: `${progress}%` }} /></div></div>
      </div>
      <div className="bg-green-950/50 backdrop-blur-lg rounded-lg border border-green-800/30 p-6">
      ...
      <div className="bg-green-950/50 backdrop-blur-lg rounded-lg border border-green-800/30 p-6">
        ...
        {showEvalForm && (<form onSubmit={handleCreateEvaluation} className="border border-green-800/30 rounded-lg p-4 mb-4 space-y-3"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-gray-300 mb-1">Período</label><select value={evalPeriod} onChange={(e) => setEvalPeriod(e.target.value)} className="w-full border border-green-700/40 rounded-lg px-3 py-2 text-sm"><option value="THIRTY">30 dias</option><option value="SIXTY">60 dias</option><option value="NINETY">90 dias</option></select></div><div><label className="block text-sm font-medium text-gray-300 mb-1">Nota (1-10)</label><input type="number" value={evalScore} onChange={(e) => setEvalScore(Number(e.target.value))} className="w-full border border-green-700/40 rounded-lg px-3 py-2 text-sm" min={1} max={10} /></div></div><div><label className="block text-sm font-medium text-gray-300 mb-1">Comentário</label><textarea value={evalComment} onChange={(e) => setEvalComment(e.target.value)} className="w-full border border-green-700/40 rounded-lg px-3 py-2 text-sm" rows={3} /></div><div className="flex gap-2"><button type="submit" className="px-4 py-2 bg-green-700 text-white rounded-lg hover:bg-green-800 text-sm">Salvar Avaliação</button><button type="button" onClick={() => setShowEvalForm(false)} className="px-4 py-2 text-gray-400 text-sm">Cancelar</button></div></form>)}
